...
function hideNotTop(){
    console.log("hidenottop")
    parent = document.getElementById("chat")
    if (topDonator && topDonator.id){
        console.log("hidenottop1")
        var children = parent.children;
        for (var i = 0; i < children.length; i++) {
          if (children[i].dataset.userID != topDonator.id) {
...
function handleChat(channel, user, message, self) {
    ...
    if (user.bits) {
        if (donatorsList[userID]) {
            ...
            donatorsList[userID].name = user.username
        } else {
            donatorsList[userID] = {'amount' : parseInt(user.bits), 'name' : user.username}
        }

        topAmount = topDonator.amount ? topDonator.amount : 0
        donatorsList[userID].amount = Math.min(donatorsList[userID].amount, topAmount + 500)


        if (!topDonator.amount || topDonator.amount < donatorsList[userID].amount) {
            topDonator.amount = donatorsList[userID].amount
            topDonator.name = name
            topDonator.id = userID
        }
    }
    ...
    chatLine.dataset.userID = userID;
